(() => {
  ...
  function bindEvents() {
    ...
    $('btn-guardar-producto')?.addEventListener('click', async () => { const body = { id:$('prod-edit-id')?.value, codigo_barras:$('prod-codigo')?.value, descripcion:$('prod-nombre')?.value, precio_venta:$('prod-precio')?.value, stock_minimo:$('prod-stock-min')?.value, id_categoria:$('prod-cat-sel')?.value, id_proveedor:$('prod-prov-sel')?.value || null }; if (!body.codigo_barras || !body.descripcion) return toast('Código y nombre son requeridos','error'); await apiFetch('/api/productos', { method:'POST', body }); toast('Producto guardado correctamente'); closeModal('modal-producto'); loadProductos(); });
    ...
    $('btn-procesar-venta')?.addEventListener('click', async () => { if (!state.cart.length) return toast('El carrito está vacío','error'); const { data } = await apiFetch('/api/ventas', { method:'POST', body:{ items:state.cart, metodo:$('pos-metodo')?.value, cliente_dni:$('cliente-doc')?.value?.trim() || '' } }); toast(`Venta procesada - Total: ${fmt(data.total || data.total_pagar)}`); state.cart = []; renderCart(); if ($('cliente-doc')) $('cliente-doc').value = ''; });
    ...
    // Botones de Ventas
    $('btn-filtrar-ventas')?.addEventListener('click', () => loadVentas());
    $('btn-exportar-ventas')?.addEventListener('click', () => exportarVentas());
    // Botones de Reportes
    $('btn-generar-reporte')?.addEventListener('click', () => loadReporte());
    $('btn-exportar-csv')?.addEventListener('click', () => exportCSV());
    // Botón de Auditoría
    $('btn-filtrar-auditoria')?.addEventListener('click', () => loadAuditoria());
  }
